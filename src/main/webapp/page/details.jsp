...
<DIV class=lefter>
<H1>${house.title}</H1>
<DIV class=subinfo>${house.adddate}</DIV>
<DIV class=houseinfo>
<P>户　　型：<SPAN>${house.houseType.htypeName}</SPAN></P>
<P>面　　积：<SPAN>${house.floorage}m<SUP>2</SUP></SPAN></P>
<P>位　　置：<SPAN>${house.street.district.districtName}${house.street.streetName}</SPAN></P>
<P>联系方式：<SPAN>${house.houseUser.telephone}</SPAN></P></DIV></DIV>
<DIV class=side>
...
<DIV class=content>
<c:if test="${house.housePictures != null}">
<c:forEach items="${house.housePictures}" var="pic">
    <img src="${rt}${pic.url}" width="100px">
</c:forEach>
</c:if>
